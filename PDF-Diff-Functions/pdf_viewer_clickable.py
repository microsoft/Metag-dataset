...
# pip install Pillow klembord tkinterdnd2 pywin32 pymupdf pyautogui
# Note: pywin32 in WSL allows COM interop with Windows applications
# Make sure you have an X server running (e.g., VcXsrv, X410) and DISPLAY is set
#
...
def convert_word_to_pdf_no_markup(input_file_path, output_pdf_path=None):
	...
	if is_wsl:
		try:
			# Convert /mnt/c/... to C:\... or WSL path to Windows path
			if input_file_path.startswith('/mnt/'):
				parts = input_file_path.split('/')
				drive = parts[2].upper()
				path = '/'.join(parts[3:])
				input_file_path = f"{drive}:\\{path.replace('/', '\\')}"
			elif not input_file_path[1:3] == ':\\':
				# Use wslpath to convert
				result = subprocess.run(['wslpath', '-w', input_file_path], capture_output=True, text=True)
				if result.returncode == 0:
					input_file_path = result.stdout.strip()
			
			if output_pdf_path:
				if output_pdf_path.startswith('/mnt/'):
					parts = output_pdf_path.split('/')
					drive = parts[2].upper()
					path = '/'.join(parts[3:])
					output_pdf_path = f"{drive}:\\{path.replace('/', '\\')}"
				elif not output_pdf_path[1:3] == ':\\':
					result = subprocess.run(['wslpath', '-w', output_pdf_path], capture_output=True, text=True)
					if result.returncode == 0:
						output_pdf_path = result.stdout.strip()
		except Exception as e:
			print(f"Warning: Could not convert WSL path: {e}")
	else:
...
class PDFViewerPane:
	...
	def _find_word_at_position(self, x_content, y_content):
		...
		# Determine which page we're on based on y_content
		for page_num, page_info in self.page_layout_info.items():
			page_y_start = page_info["y_start_offset"] * self.zoom_level
			page_y_end = page_y_start + page_info["base_height"] * self.zoom_level
			
			if page_y_start <= y_content <= page_y_end:
				...
				# Convert content coordinates to page coordinates
				page_x = (x_content - x_offset) / self.zoom_level
				page_y = (y_content - page_y_start) / self.zoom_level
				
				# Search for word at this position on this page
				for word in self.words_data:
					if word["page_num"] == page_num:
						if (word["x0"] <= page_x <= word["x1"] and
							word["y0"] <= page_y <= word["y1"]):
							return word
				break
		return None

	def _get_diff_context(self, clicked_word):
		"""Gets the context around a clicked diff word (consecutive highlighted words)."""
		if not self.words_data:
			return {"words": [clicked_word], "text": clicked_word["text"]}
		
		# Find the index of the clicked word
		clicked_idx = None
		for idx, word in enumerate(self.words_data):
			if (word["page_num"] == clicked_word["page_num"] and
				word["x0"] == clicked_word["x0"] and
				word["y0"] == clicked_word["y0"] and
				word["text"] == clicked_word["text"]):
				clicked_idx = idx
				break
		...
		highlight_color = clicked_word.get("highlight_color")
		
		# Find the start of the consecutive highlighted section
		start_idx = clicked_idx
		while start_idx > 0:
			prev_word = self.words_data[start_idx - 1]
			if prev_word.get("highlight_color") == highlight_color:
				start_idx -= 1
			else:
				break
		
		# Find the end of the consecutive highlighted section
		end_idx = clicked_idx
		while end_idx < len(self.words_data) - 1:
			next_word = self.words_data[end_idx + 1]
			if next_word.get("highlight_color") == highlight_color:
				end_idx += 1
			else:
				break
		
		# Extract the consecutive highlighted words
		context_words = self.words_data[start_idx:end_idx + 1]
		context_text = " ".join([w["text"] for w in context_words])
		
		# Also get some surrounding context (non-highlighted words)
		context_before = []
		for i in range(max(0, start_idx - 5), start_idx):
			context_before.append(self.words_data[i]["text"])
		
		context_after = []
		for i in range(end_idx + 1, min(len(self.words_data), end_idx + 6)):
			context_after.append(self.words_data[i]["text"])
		...
		return {
			...
			"page_num": clicked_word["page_num"]
		}

	def _show_save_diff_dialog(self, clicked_word, diff_context):
		"""Shows a confirmation dialog for saving the diff to JSONL."""
		highlight_color = diff_context.get("highlight_color", "unknown")
		diff_type = "deletion" if highlight_color == "red" else "insertion" if highlight_color == "green" else "moved"
		
		# Build the confirmation message
		message = f"Save this {diff_type} to JSONL file?\n\n"
		message += f"Type: {diff_type.upper()}\n"
		message += f"Page: {diff_context.get('page_num', 0) + 1}\n"
		message += f"Pane: {self.pane_id.upper()}\n\n"
		
		if diff_context.get("context_before"):
			message += f"...{diff_context['context_before']} "
		
		message += f"[{diff_context['text']}]"
		
		if diff_context.get("context_after"):
			message += f" {diff_context['context_after']}..."
		
		message += f"\n\nOutput file: {DIFF_OUTPUT_FILE}"
		
		# Show confirmation dialog
		if messagebox.askyesno("Save Diff", message):
			self._save_diff_to_jsonl(diff_context)

	def _save_diff_to_jsonl(self, diff_context):
		"""Saves the diff context to the JSONL file."""
		import datetime
		...
		try:
			...
			messagebox.showinfo("Saved", f"Diff successfully saved to:\n{DIFF_OUTPUT_FILE}")
		except Exception as e:
			print(f"Error saving diff: {e}")
			messagebox.showerror("Save Error", f"Failed to save diff: {e}")
